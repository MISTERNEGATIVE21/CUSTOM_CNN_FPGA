...
def run_keras_workflow():
    ...
    print("--- Building Deeper QKeras Model ---")
    ...
    # --- FINAL FIX for Keras/QKeras incompatibility ---
    model.compile(
        optimizer='adam',
        loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
        metrics=['accuracy'],
        run_eagerly=True # Force eager execution to prevent .numpy() error
    )
    model.summary()
...
def run_pytorch_workflow():
    ...
    config = hls4ml.utils.config_from_onnx_model(qonnx_model, granularity='name', default_precision='ap_fixed<16,6>')

    # --- FINAL FIX for 'g++: Killed' out-of-memory error ---
    config['Model']['ReuseFactor'] = 16
    print(f"--- hls4ml config updated with ReuseFactor={config['Model']['ReuseFactor']} to prevent memory issues ---")

    print("\n--- Converting prepared QONNX model to HLS project ---")
    hls_model = hls4ml.converters.convert_from_onnx_model(qonnx_model, output_dir='hls4ml_pytorch_project', hls_config=config)
